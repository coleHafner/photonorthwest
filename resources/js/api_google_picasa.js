...
function pwaLoadPhotoGrid( type, query, callback, user )
{
    ...
    switch( type )
    {
        ...
        case "search":
            ...
            photo_feed_url = search_url.replace( "%query%", query ).replace('%max%', 100);
...
function pwaGetFeedUrls()
{
    ...
    urls.search = 'https://picasaweb.google.com/data/feed/api/all?q=%query%&alt=json&max-results=%max%';
